...
## 关于如何进行项目开发时间预估
1. 首先确认个人所负责的模块（这里有能力的话，需要考虑与之相关联的功能模块）。
2. 确认UI图与产品文档是否有出入，记录不理解的部分。
3. 找PM单独过一遍产品原型（如果可以的话，可以与后台开发人员一起参与）。
4. 确认已知开发内容之后进行开发时间评估。
5. 如果存在问题点及时抛出，如果存在可能会影响deadline发版的问题及时与负责人进行沟通。

## 项目上线规范
>目前部署方式，和ci部分还未调整完成。所以该部分可能会后期进行修改。

1. 构建正式tag首先更新当前分支以及从master上合并，保证当前分支为最新。
2. 上线完成之后，需要将开发分支合并至master上。
3. 删除开发分支。
